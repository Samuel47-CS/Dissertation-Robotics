...
python -m lerobot.async_inference.robot_client \
    ...
    --robot.type=bi_so101_follower \ # ROBOT: your robot type
    ...
    --task="Horizontally fold the yellow cloth" \ # POLICY: The task to run the policy on (`Fold my t-shirt`). Not necessarily defined for all policies, such as `act`
    --policy_type=smolvla \ # POLICY: the type of policy to run (smolvla, act, etc)
    --pretrained_name_or_path=the-sam-uel/folding-no-style-4-batch \ # POLICY: the model name/path on server to the checkpoint to run (e.g., lerobot/smolvla_base)
